...
def load_configurations(path: Path) -> list[tuple[str | None, str | None]]:
    ...
    for raw in path.read_text(encoding="utf-8", errors="ignore").splitlines():
        ...
        scen = (
            None
            if (scen_raw is None or str(scen_raw).strip() == "None")
            else str(scen_raw).strip()
        )
        plat = (
            None
            if (plat_raw is None or str(plat_raw).strip() == "None")
            else str(plat_raw).strip()
        )
...
def main() -> int:
    ...
    if body.strip():
        ...
        print(
            "No content to post (no owners matched, no platform-only items, "
            "and no unowned items)."
        )
    return 0
